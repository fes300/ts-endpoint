...
import { IOError } from 'ts-io-error/lib/index';
import { type RESTEndpoints } from './types/RESTEndpoints';
import { APIRESTClient } from 'ts-endpoint-ra-data-provider/lib/APIRESTClient';
  ...
  return pipe(
    ...
    TE.chain((content) => {
      return pipe(
        ...
        E.mapLeft(
          (e) =>
            new IOError(`Validation Failed for codec`, {
              kind: 'DecodingError',
...
export type GetFnParams<G> = InferEndpointParams<G>['params'] extends t.ExactType<infer T>
  ? t.TypeOf<T>
...
export type GetListFnQuery<G> = InferEndpointParams<G>['query'] extends t.ExactType<infer T>
  ? t.TypeOf<T>
  : InferEndpointParams<G>['query'] extends undefined
  ? undefined
  : serializedType<InferEndpointParams<G>['query']>;

export type EndpointOutput<L> = InferEndpointParams<L>['output'] extends t.ExactType<infer T>
  ? t.TypeOf<T>['data'] extends any[]
    ? t.TypeOf<T>
    : t.TypeOf<T>['data']
  : never;

export type EndpointDataOutput<L> = InferEndpointParams<L>['output'] extends t.ExactType<infer T>
  ? t.TypeOf<T>
  : never;

export type GetDataOutputEI<L> = InferEndpointInstanceParams<L>['output'] extends t.ExactType<
  infer T
>
  ? t.TypeOf<T>['data'] extends any[]
    ? t.TypeOf<T>
    ...
    ? {
        [K in keyof CC]: (
          params: (InferEndpointInstanceParams<CC[K]>['headers'] extends t.Mixed
            ? {
                Headers: serializedType<InferEndpointInstanceParams<CC[K]>['headers']>;
              }
            : Record<string, unknown>) &
            (InferEndpointInstanceParams<CC[K]>['query'] extends t.Mixed
              ? {
                  Query: serializedType<InferEndpointInstanceParams<CC[K]>['query']>;
                }
              : Record<string, unknown>) &
            (InferEndpointInstanceParams<CC[K]>['params'] extends t.Mixed
              ? {
                  Params: serializedType<InferEndpointInstanceParams<CC[K]>['params']>;
                }
              : Record<string, unknown>) &
            (InferEndpointInstanceParams<CC[K]>['body'] extends undefined
              ? Record<string, unknown>
              : {
                  Body: serializedType<InferEndpointInstanceParams<CC[K]>['body']>;
                })
...
type EndpointsRESTClient<ES> = {
  [K in keyof ES]: ES[K] extends RESTEndpoints<
...
>(
  ...
  return {
    ...
    getList: (
      ...
      InferEndpointParams<L>['output'] extends t.ExactType<infer T> ? t.TypeOf<T> : never
    > => {
      return pipe(
        liftFetch(
          () =>
...
export { GetEndpointsRESTClient, type EndpointsRESTClient };
