...
interface StructType extends t.InterfaceType<{ [K: string]: t.TypeOf<HasOpenAPISchema> }> {}
...
export const getOpenAPISchema = <T extends IOTOpenDocSchema>(codec: T): any => {
  ...
  switch (type._tag) {
    ...
    case 'ArrayType':
      return {
        type: 'array',
        description: type.name.replace('Array<', '').replace('>', '').concat('[]'),
        items: [
          {
    ...
    case 'PartialType':
    case 'ExactType': {
      const properties = record.map(getProps(type), getOpenAPISchema as any) as any;
      return {
        type: 'object',
    ...
    case 'UnionType':
      ...
      if (nonNullableTypes.every((v: any) => v._tag === 'LiteralType')) {
        return {
          type: 'string',
          description: type.name,
          enum: (nonNullableTypes as Array<t.LiteralType<any>>).map((t) => t.value),
          required: isRequired,
        };
      }
...
export const getOpenAPISchemaFromModel =
  <M extends { [key: string]: any }>(models: M) =>
  <C extends HasOpenAPISchema>(codec: C): any => {
    console.debug('models', models);
    const ref = Object.values(models).find((m) => m.name === codec.name);
    if (ref) {
      return ref;
    }

    return getOpenAPISchema(codec);
  };
